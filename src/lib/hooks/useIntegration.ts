...
export function useIntegration(): UseIntegrationReturn {
  ...
  const loadIntegrations = async () => {
    try {
      ...
      if (user) {
        const userIntegrations = await integrationService.getIntegrations(
          user.uid
        );
  ...
  const connectIntegration = async (
    ...
    try {
      ...
      const success = await integrationService.connectIntegration(
        user.uid,
        type,
        config
      );
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Erreur lors de la connexion à l'intégration"
      );
  ...
  const disconnectIntegration = async (
    ...
    try {
      ...
      const success = await integrationService.disconnectIntegration(
        user.uid,
        type
      );
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Erreur lors de la déconnexion de l'intégration"
      );
  ...
  const triggerZapierWebhook = async (
    ...
    try {
      ...
      const result = await integrationService.triggerZapierWebhook(
        user.uid,
        eventType
      );
      return result;
      ...
      const errorMessage =
        err instanceof Error
          ? err.message
          : "Erreur lors du déclenchement du webhook Zapier";
      setError(errorMessage);
  ...
  const backupToGoogleDrive = async (
    ...
    try {
      ...
      const result = await integrationService.backupToGoogleDrive(
        user.uid,
        fileData,
        fileName
      );
