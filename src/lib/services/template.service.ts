...
export class TemplateService {
  ...
  static async getTemplate(templateId: string): Promise<DocumentTemplate | null> {
    try {
      const templateDoc = await getDoc(doc(db, 'documentTemplates', templateId));
  ...
  static async createTemplate(template: Omit<DocumentTemplate, 'id' | 'createdAt' | 'updatedAt'>): Promise<string> {
    try {
      console.log("TemplateService.createTemplate - Début de la création du template:", template.title);
      console.log("Contenu du template (premiers 200 caractères):", template.content.substring(0, 200) + "...");
      
      // Extraire les URLs d'images du contenu HTML
      const imageUrls = template.imageUrls || this.extractImageUrls(template.content);
      ...
      const docRef = await addDoc(collection(db, 'documentTemplates'), newTemplate);
  ...
  static async updateTemplate(templateId: string, template: Partial<Omit<DocumentTemplate, 'id' | 'createdAt' | 'updatedAt'>>): Promise<boolean> {
    try {
  ...
  static async deleteTemplate(templateId: string): Promise<boolean> {
    try {
      ...
      if (template && template.imageUrls && template.imageUrls.length > 0) {
        await Promise.all(template.imageUrls.map(url => {
  ...
  static async uploadImage(file: File, userId: string): Promise<string> {
    try {
      console.log("Début du téléchargement d'image:", { fileName: file.name, fileSize: file.size, fileType: file.type });
  ...
  private static extractImageUrls(content: string): string[] {
    ...
    try {
      console.log("Extraction des URLs d'images à partir du contenu:", content.substring(0, 100) + "...");
      const urls: string[] = [];
      ...
      while ((match = regex.exec(content)) !== null) {
        if (match[1]) {
          if (!match[1].startsWith('data:')) { // Ignorer les images en base64
