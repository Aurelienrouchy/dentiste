...
class AIService {
  private availableModels: AIModel[] = [
    { id: 'whisper', name: 'OpenAI Whisper', description: 'Modèle de reconnaissance vocale performant pour la transcription audio', typeNatif: 'audio' },
    { id: 'gpt4', name: 'GPT-4', description: 'Analyse les transcriptions et génère des documents médicaux structurés', typeNatif: 'texte' },
    { id: 'gpt4o', name: 'GPT-4o', description: 'Modèle multimodal pouvant traiter le texte et l\'audio', typeNatif: 'multimodal' },
    { id: 'claude3', name: 'Anthropic Claude 3', description: 'Modèle d\'IA conçu pour respecter les protocoles médicaux et la confidentialité', typeNatif: 'texte' },
    { id: 'gemini', name: 'Google Gemini', description: 'Modèle avancé de Google pour le traitement du langage naturel médical', typeNatif: 'multimodal' },
  ];
  ...
  getTranscriptionModels(): AIModel[] {
    return this.availableModels.filter(model => 
      model.typeNatif === 'audio' || model.typeNatif === 'multimodal'
    );
  }
  ...
  getDocumentModels(): AIModel[] {
    return this.availableModels.filter(model => 
      model.typeNatif === 'texte' || model.typeNatif === 'multimodal'
    );
  }
   ...
   */
  async transcribeAudio(audioBlob: Blob, modelId: string): Promise<TranscriptionResult> {
    try {
      ...
      throw new Error('Erreur lors de la transcription audio. Veuillez réessayer.');
    }
  }
  ...
  async generateDocument(
    ...
    try {
      ...
      if (templateContent) {
        ...
        systemPrompt = `Vous êtes un assistant spécialisé dans la création de documents médicaux dentaires professionnels. 
        Générez un document de type ${config.type.replace(/-/g, ' ')} basé sur la transcription fournie.
        Format: professionnel, précis, conforme aux standards médicaux.
        Patient: ${config.patientName || "Patient"}
        Praticien: ${config.practitionerName || "Dr. Exemple"}
        Date: ${config.date || new Date().toLocaleDateString()}`;
      ...
      const apiEndpoint = modelId === 'gpt4o' 
        ? 'gpt-4o-2024-05-13' 
        : 'gpt-4-turbo-preview';
      ...
      const response = await fetch('https://api.openai.com/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          messages: [
            ...
            {
              role: 'user',
              content: "Voici ma transcription: " + (typeof document.getElementById === 'function' ? document.getElementById('transcription')?.innerText : '')
            }
          ],
          temperature: 0.3
        })
      ...
      throw new Error('Erreur lors de la génération du document. Veuillez réessayer.');
    }
  }
}
...
export function useAIService() {
  ...
  const startRecording = async (): Promise<void> => {
    try {
      ...
      setError(err.message || 'Impossible de démarrer l\'enregistrement');
  ...
  const generateDocument = async (
    ...
    try {
      ...
      const document = await aiService.generateDocument(config, modelId, templateContent);
