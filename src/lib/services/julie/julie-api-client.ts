...
export class JulieApiClient {
  ...
  async testConnection(): Promise<boolean> {
    try {
      const response = await this.apiClient.get<
        JulieApiResponse<{ status: string }>
      >("/status");
  ...
  private formatPatientForJulie(patient: ExportablePatient): any {
    ...
    return {
      ...
      civility:
        patient.gender === "Madame"
          ? "Mme"
          : patient.gender === "Monsieur"
          ? "M"
          : "",
  ...
  async exportPatients(patients: ExportablePatient[]): Promise<SyncResult> {
    try {
      ...
      const response = await this.apiClient.post<
        JulieApiResponse<{ imported: number }>
      >("/patients/import", {
      ...
      if (response.data.success) {
        return {
          success: true,
          message: `${
            response.data.data?.imported || patients.length
          } patients exportés avec succès vers Julie`,
          timestamp: new Date().toISOString(),
          itemsProcessed: response.data.data?.imported || patients.length,
        };
      } else {
        throw new Error(
          response.data.error?.message || "Échec de l'export vers Julie"
        );
      }
    } catch (error) {
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Erreur inconnue lors de l'export vers Julie";
      console.error(
        "Erreur lors de l'export des patients vers Julie:",
        errorMessage
      );
      ...
      if (response.data.success) {
        ...
        throw new Error(
          response.data.error?.message ||
            "Échec de la récupération des patients depuis Julie"
        );
      }
    } catch (error) {
      console.error(
        "Erreur lors de la récupération des patients depuis Julie:",
        error
      );
      throw error;
    }
  ...
  async getAppointments(startDate: string, endDate: string): Promise<any[]> {
    try {
      const response = await this.apiClient.get<
        JulieApiResponse<{ appointments: any[] }>
      >("/appointments", {
      ...
      if (response.data.success) {
        ...
        throw new Error(
          response.data.error?.message ||
            "Échec de la récupération des rendez-vous depuis Julie"
        );
      }
    } catch (error) {
      console.error(
