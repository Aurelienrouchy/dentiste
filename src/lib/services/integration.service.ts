...
export class IntegrationService {
  ...
  private availableIntegrations: Partial<Integration>[] = [
    {
      ...
      name: "Julie",
      description:
        "Julie Solutions améliore votre quotidien avec le meilleur logiciel de gestion de cabinet dentaire.",
    ...
    },
    {
      ...
      name: "Logosw",
      description:
        "Optimisez votre cabinet dentaire, gagnez du temps grâce à votre logiciel. LOGOSw est le premier logiciel pour cabinet dentaire agréé CCAM.",
    ...
    },
    {
      ...
      name: "Zapier",
      description:
        "Logiciel d'automatisation des flux de travail pour tous. Automatisez votre travail sans développeurs, sans délais.",
    ...
    },
    {
      ...
      name: "Weclever",
      description:
        "WeClever Dental, LE logiciel de gestion de cabinet dentaire 100% cloud qui offre une accessibilité et une simplicité remarquables.",
    ...
    },
    {
      ...
      name: "Veasy",
      description:
        "Optimisez votre pratique dentaire avec Veasy, le logiciel dentaire tout-en-un pour la gestion de votre cabinet dentaire et centre de santé.",
    ...
    },
    {
      ...
      name: "Google Drive",
      description:
        "Un stockage cloud évolutif et facile à utiliser, adapté à tous les collaborateurs et aux équipes de toutes tailles.",
    ...
    try {
      ...
      switch (integrationType) {
        ...
        default:
          ...
          connectionSuccess = await this.simulateExternalApiConnection(
            integrationType,
            config
          );
          break;
  ...
  async exportPatients(
    ...
    try {
      ...
      if (!integrationDoc.exists() || !integrationDoc.data().isConnected) {
        throw new Error(
          `L'intégration ${integrationType} n'est pas connectée.`
        );
      }
      ...
      switch (integrationType) {
        case "julie":
          ...
          if (!this.apiClients.julie) {
            ...
            this.apiClients.julie = new JulieApiClient({
              apiKey: config.apiKey,
            });
          }

          // Utiliser le client Julie pour l'export
          result = await this.apiClients.julie.exportPatients(
            exportablePatients
          );
          break;
        ...
        default:
          // Pour les autres intégrations, utiliser la simulation pour l'instant
          result = await this.simulateExport(
            integrationType,
            exportablePatients
          );
          break;
      ...
      console.error(
        `Erreur lors de l'export des patients vers ${integrationType}:`,
        error
      );
      return {
        success: false,
  ...
  private convertToExportablePatient(patient: Patient): ExportablePatient {
    ...
    const birthDateString = `${year}-${month.padStart(2, "0")}-${day.padStart(
      2,
      "0"
    )}`;
  ...
  async disconnectIntegration(
    ...
    try {
      ...
      if (!integrationDoc.exists()) {
        throw new Error(
          `L'intégration ${integrationType} n'existe pas pour cet utilisateur.`
        );
      }
      ...
      console.error(
        `Erreur lors de la déconnexion de ${integrationType}:`,
        error
      );
      throw error;
    }
  ...
  async triggerZapierWebhook(
    ...
    try {
      ...
      return {
        success: false,
        message: `Échec du déclenchement du webhook: ${
          error instanceof Error ? error.message : "Erreur inconnue"
        }`,
        timestamp: new Date().toISOString(),
        errors: [error instanceof Error ? error.message : "Erreur inconnue"],
  ...
  async backupToGoogleDrive(
    ...
    console.log("backupToGoogleDrive", userId, fileData, fileName);
    try {
      // Vérifier si l'intégration est connectée
      const integrationRef = doc(
        db,
        `users/${userId}/integrations/google_drive`
      );
      const integrationDoc = await getDoc(integrationRef);
  ...
  async importPatientsFromJulie(userId: string): Promise<SyncResult> {
    try {
      ...
      console.error(
        `Erreur lors de l'import des patients depuis Julie:`,
        error
      );
      return {
        success: false,
  ...
  async getAppointmentsFromJulie(
    ...
    try {
      ...
      // Récupérer les rendez-vous depuis Julie
      const appointments = await this.apiClients.julie.getAppointments(
        startDate,
        endDate
      );
      ...
      console.error(
        `Erreur lors de la récupération des rendez-vous depuis Julie:`,
        error
      );
      return {
        success: false,
