...
import { 
  Bold, Italic, List, ListOrdered, Heading, Code, Undo, Redo, 
  Image as ImageIcon, Loader2, AlignLeft, AlignCenter, AlignRight, 
  AlignJustify, Table as TableIcon, Underline as UnderlineIcon, Type, Printer,
  Palette, Divide
} from 'lucide-react';
...
export function TemplateEditor({ content, onChange, placeholder = 'Commencez à rédiger votre template...' }: TemplateEditorProps) {
  const { user } = useAuth();
  ...
  const [selectedHeadingLevel, setSelectedHeadingLevel] = useState<string>('paragraph');
  ...
  const addImage = async () => {
    if (!editor || !user) {
      ...
      setUploadError("Éditeur ou utilisateur non disponible. Veuillez vous reconnecter.");
      return;
    }
    ...
    if (activeTab === 'url' && imageUrl) {
      try {
        ...
        // Approche correcte pour insérer une image dans TipTap
        ...
        setUploadError("Erreur lors de l'insertion de l'image. Format d'URL invalide ou inaccessible.");
      }
      ...
      try {
        console.log("Préparation du téléchargement de l'image:", uploadedImage.name);
        // Télécharger l'image vers Firebase Storage
        const imageUrl = await TemplateService.uploadImage(uploadedImage, user.uid);
        console.log("Image téléchargée avec succès, URL reçue:", imageUrl);
        
        // Insérer l'image avec l'URL générée par Firebase
        ...
        console.error('Erreur détaillée lors du téléchargement ou de l\'insertion de l\'image:', error);
        setUploadError(`Erreur: ${error instanceof Error ? error.message : 'Problème inconnu'}`);
      } finally {
        setIsUploading(false);
      }
    } else {
      console.warn("Aucune image sélectionnée ou URL fournie");
      setUploadError("Veuillez sélectionner une image ou fournir une URL valide.");
    }
  };
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (e.target.files && e.target.files[0]) {
      ...
      if (file.size > maxSizeInBytes) {
        setUploadError('L\'image est trop volumineuse. La taille maximale est de 5 Mo.');
        return;
      }
  ...
  return (
    <div className="border rounded-md bg-white">
              ...
              {showColorPicker && (
                <div className="absolute z-50 mt-1 p-2 bg-white border rounded-md shadow-lg">
                  <div className="grid grid-cols-5 gap-1">
                    {['#000000', '#e60000', '#0066cc', '#008a00', '#9933cc', 
                      '#ff9900', '#0099ff', '#8e8e8e', '#ffffff', '#333333'].map(color => (
                      <Button
                        key={color}
                        variant="ghost"
      ...
      {/* Éditeur */}
      <div className="p-6 min-h-[400px] prose max-w-none" style={{ backgroundColor: '#ffffff' }}>
        <EditorContent editor={editor} className="outline-none" />
      </div>
      
      {/* Barre de statut */}
      <div className="p-2 border-t bg-gray-50 text-xs text-muted-foreground flex justify-between">
        <p>Utilisez <code className="bg-muted rounded px-1">[transcription]</code> pour indiquer où la transcription sera insérée.</p>
        <div>
